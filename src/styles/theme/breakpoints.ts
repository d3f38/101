// Reference https://github.com/mui-org/material-ui/blob/master/packages/material-ui/src/styles/createBreakpoints.js
...
function createBreakpoints() {
  ...
  function down(key: keyof typeof values) {
    ...
    if (endIndex === keys.length) {
      // lg down applies to all sizes
      return up('xs')
    }

    const value =
      typeof upperbound === 'number' && endIndex > 0 ? upperbound : values[key]
  ...
  function between(start: keyof typeof values, end: keyof typeof values) {
    ...
    return (
      ...
      `(max-width:${
        (endIndex !== -1 ? values[keys[endIndex + 1]] : values[end]) -
        step / 100
      }${unit})`
    )
  }
